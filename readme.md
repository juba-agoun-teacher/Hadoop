...
- Écrire un simple programme MapReduce en Python.
...
Cela peut être Windows si vous êtes sur les machines de l'IUT et ça peut être aussi macOS dans le cas d'un ordinateur Apple.
...
- Mémoire dédiée au cluster (Minimum : 4 GiB, Recommandée : 8+ GiB). Plus c'est mieux.
- CPU (Minimum : 4 Cores, Recommandés : 6+ Cores)
...
Dans la fenêtre suivante, vous devrez peut-être modifier certains paramètres. Veillez à régler les cœurs du processeur sur 4 et la taille de la mémoire vive sur 4 Go au minimum, et idéalement 8 Go pour les ordinateurs performants.
...
Une fois l'opération de démarrage terminée, vous verrez l'écran suivant qui donne l'adresse pour accéder à la page d'accueil de la plateforme à l'adresse http://localhost:1080 ou http://127.0.0.1:1080 pour HDP 2.6.5.
